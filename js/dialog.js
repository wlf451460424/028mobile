...
(function(root, factory) {
	...
	Dialog.prototype.init=function(settings) {
			var _this = this;
		...
		Dialog.prototype.touch= function(obj, fn) {
			var move;
			$(obj).on('click', click);
	...
	$.alert = function(content, button, callback, timer, settings) {
			...
			return $.Dialog($.extend(options, settings));
		};
		/*
		buttons :[{yes:"确定"},{no:'取消'},{close:'关闭'}]
		*/
	$.Dateconfirm = function(content, buttons, callback, settings) {
		var options = {};
		var defaults = {
			zIndex: 1000,
			type: 'confirm'
