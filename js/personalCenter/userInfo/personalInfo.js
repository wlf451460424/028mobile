...
function personalInfoInit(){
	localStorageUtils.setParam("intoType","");
	
    userName = localStorageUtils.getParam("username");
	myUserID = localStorageUtils.getParam("myUserID");

    var param = '{"ProjectPublic_PlatformCode":2,"UserName":"' + userName + '","InterfaceName":"/api/v1/netweb/GetUserDetailNew"}';
    ajaxUtil.ajaxByAsyncPost1(null, param, successCallBack_personaInfo,null);   
    
    var paramPwd = '{"ProjectPublic_PlatformCode":2,"UserName":"' + userName + '","InterfaceName":"/api/v1/netweb/CheckPayOutPwdAndTransferPwd"}';
    ajaxUtil.ajaxByAsyncPost1(null, paramPwd, searchSuccessCallBack_personaInfo,null);

	// FlagType 类型（1：密保 2：提示语）
	var paramSecurity = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"/api/v1/netweb/GetSecurityPromptingState","UserID":' + myUserID + ',"FlagType":1}';
...
function successCallBack_personaInfo(data) {
	if (data.Code == 200) {
		realName = data.Data.UserRealName;
		...
		var userFandian = data.Data.MyRebate;
...
function loadSetSecurityPage(nextPage) {
	if (securityState){
		var Security = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"/api/v1/netweb/GetRandomSecurity","UserID":' + myUserID + ',"FlagType":1,"SecurityCount":2}';
							...
							doneCallback:
								function(){
									...
									if (randomAnswer.trim() == ""){
										...
										var paramAnwser = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"/api/v1/netweb/RetrievePassword","UserName":"' + userName + '","RecoveryMode":2,"SecurityID":'+ info.ID +',"Answer":"'+ randomAnswer +'"}';
										ajaxUtil.ajaxByAsyncPost1(null, paramAnwser, function (data) {
											if (data.Code == 200) {
												if (data.Data.Result == 1){
