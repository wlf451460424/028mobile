    ...
    $.fn.imgcode = function (options) {
        ...
        return this.each(function () {
            ...
            var html = '<div class="code-k-div">' +
                ...
                '<div class="code-mask" style="top:'+options.pointY+'px;">'+
                '<img class="code-front-img" src="data:image/gif;base64,'+opts.frontimg+'"></div>' +
                '<img class="code-back-img" src="data:image/gif;base64,'+opts.backimg+'"></div>' +
                '<div class="clearfix code-tips"><span class="refreshIcon" alt="刷新验证码"></span><span id="valid_errorMsg" class="valid-error-msg"></span><span id="valid_sucMsg" class="valid-suc-msg"></span></div>' +
                '</div>' +
                '<div class="code-btn">' +
                '<div class="code-btn-slide-indicator"></div>'+
                '<div class="code-btn-img code-btn-m"><span class="code-btn-slider-icon"></span></div>' +
                '<span class="code-span">向右拖动滑块填充拼图</span>' +
                '</div></div></div>';
            ...
            $divWrap.each(function(index, el) {
            	el.addEventListener('touchmove', function (event) {
		    		event.preventDefault();
		     	}, false);
			})
            //滑动事件
            $divWrap.unbind("touchmove").on({
            	touchmove:function (e) {
	                ...
	                if (isDown) {
	                    if(x>(dX + 20) && x < dX + $(this).width() - 20){
	                        ...
	                        // if (){}
	                        $divWrap.find(".code-btn-slide-indicator").css("width", (x - dX-20) + "px");
