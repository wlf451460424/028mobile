    ...
    ui.prototype = {
        ...
        */
        ...
        loadContent: function(target, newTab, back, transition, anchor) {
             ...
             *  当页面加载时关闭所有提示框。
             */
            $(".afPopup").each(function() {
                var popupObj = $(this).data("popupObj");
                if(popupObj){
                    popupObj.hide();
                }
            });
            ...
            if (this.doingTransition) {
                var that = this;
                this.urlHistory = this.getPanelId(document.location.hash);
        ...
        loadDiv: function(target, newTab, back, transition) {
            ...
            /********************************************************
                为backFn方法设置参数。 add 2015-02-09
                backPageConfig: {},  {当前页面 ：对应的跳转页面}
                registerPage: "",　　需记录的跳转页面*/
            if (what === this.firstDiv.id) {
                this.backPageConfig = {};
                this.registerPage = "";
            ...
            if(!this.registerPage){
                this.registerPage = this.firstDiv.id;
            }

            if(!this.backPageConfig[what]){
                this.backPageConfig[what] = this.registerPage;
            }

            //log.v(jsonUtils.toString(this.backPageConfig));

            if(!back){
                this.registerPage = what;
            }
            /****************************   end   *****************************/
